...
void triggerTest(int argc ,char * argv[]){// 触发
    ...
    events.data.fd = STDIN_FILENO; // 放自己的描述符
    epoll_ctl(epfd , EPOLL_CTL_ADD , STDIN_FILENO ,&events );
    events.events = EPOLLIN | EPOLLET; // 读就绪 + 边缘触发 edge-triggered 
    events.data.fd = netFd; // 放自己的描述符
    epoll_ctl(epfd , EPOLL_CTL_ADD , netFd , &events);
    char buf[3] = {0};
    while(1){
...
// 服务端只做中转服务
void epollTest2(int argc ,char * argv[]){  // 服务端 断线重连 + 多人即时聊天+ 超时处理
    argsCheck(argc , 3);
    ...
    char buf[4096] = {0};
    Conn_t client[1024];
    bzero(client , 1024*sizeof(Conn_t));
    int  fdtoidx[1024]; // 通过fd 找conn中的index 
    for(auto &i : fdtoidx){
        i = -1;
    }
    int curindx = 0;// 下一个客户端下标
    time_t now;
    while(1){

        struct epoll_event readyset[1024]; //就绪集合
        int readyNum = epoll_wait(epfd , readyset , 1024 , 1000);//-1 : 不会超时  //1000 : 1s 就绪一次
        now = time(NULL);
        printf("now = %s \n",ctime(&now));
        for(int i = 0;i< readyNum ;++i){
            if(readyset[i].data.fd == sockfd){ // stdin 就绪
                ...
                client[curindx].isAlive = 1;
                client[curindx].netfd = netfd;
                client[curindx].lastActive = now;
                fdtoidx[netfd] = curindx;
                ...
                events.data.fd = netfd;
                epoll_ctl(epfd , EPOLL_CTL_ADD , netfd ,&events );
                // events.events = EPOLLIN; // 服务端只做中转服务
                // events.data.fd = STDIN_FILENO;
                // epoll_ctl(epfd , EPOLL_CTL_ADD , STDIN_FILENO ,&events );
            }else {  //除了 sockfd , 肯定是 netfd
                memset(buf,0,sizeof(buf));
                ...
                int index = fdtoidx[netfd];
                client[index].lastActive = now;
                for(int j =0; j < curindx; ++j ){
                    if(client[j].isAlive == 1 && client[j].netfd != netfd){ // 转发给还存在的， 以及排除自己
                        write(client[j].netfd , buf, ret); 
                    }
                }
       ...
       for(int i =0;i<curindx ;++i){
            if(client[i].isAlive == 1 &&   now - client[i].lastActive > 10){
                close(client[i].netfd);
